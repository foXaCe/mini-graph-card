/**
 * Intelligent caching system for mini-graph-card
 * Provides multi-level caching, data compression, and smart invalidation
 */
...
export class IntelligentCache {
  ...
  async set(key, data, ttl = this.config.defaultTTL) {
    ...
    try {
      ...
      if (this.config.enableCompression && cacheEntry.size > this.config.compressionThreshold) {
        ...
        if (compressed.size < cacheEntry.size * 0.8) { // Only use if 20%+ reduction
          cacheEntry.data = compressed.data;
          cacheEntry.compressed = true;
          cacheEntry.size = compressed.size;
          cacheEntry.originalSize = this.estimateSize(data);

          this.metrics.compressionRatio =
            (this.metrics.compressionRatio + (cacheEntry.size / cacheEntry.originalSize)) / 2;
        }
      }
    ...
    try {
      ...
      if (this.config.enableMemoryCache) {
        ...
        if (cacheEntry && this.isValid(cacheEntry)) {
          ...
          const data = await this.decompressIfNeeded(cacheEntry);
  ...
  async setMemoryCache(key, cacheEntry) {
    ...
    while (this.memoryCacheSize + cacheEntry.size > this.config.maxMemorySize) {
  ...
  async compressData(data) {
    try {
      ...
      console.warn('MGC Cache: Compression failed:', error);
      return {
        data,
        size: this.estimateSize(data)
      };
  ...
  async decompressIfNeeded(cacheEntry) {
    if (!cacheEntry.compressed) {
      return cacheEntry.data;
    }

    try {
      const decompressed = decompress(cacheEntry.data);
      return JSON.parse(decompressed);
    } catch (error) {
      console.warn('MGC Cache: Decompression failed:', error);
      return null;
    }
  }
  ...
  async cleanupDiskCache() {
    ...
    try {
      ...
      const cleanupPromises = keys.map(async (key) => {
        const entry = await this.diskCache.getItem(key);
        if (!this.isValid(entry)) {
   ...
   */
  async prefetch(entityId, currentTime) {
    // Analyze access patterns to predict what data might be needed next
    const patterns = this.analyzeAccessPatterns(entityId);

    if (patterns.predictedRanges) {
      const prefetchPromises = patterns.predictedRanges.map(range => {
        const key = this.generateKey(entityId, range.start, range.end);
        return this.get(key); // This will miss but establish the pattern
      });
  ...
  analyzeAccessPatterns(entityId) {
    // Simple pattern analysis - could be enhanced with machine learning
    const accessHistory = [];

    for (const [key, time] of this.accessTimes) {
      if (key.includes(entityId)) {
        accessHistory.push(time);
      }
    }

    accessHistory.sort((a, b) => b - a); // Most recent first

    // Predict user might want to see recent data ranges
    const now = Date.now();
    const patterns = {
      predictedRanges: [
  ...
  async invalidate(pattern) {
    ...
    if (this.diskCache) {
      try {
        ...
        const deletePromises = diskKeysToDelete.map(key =>
